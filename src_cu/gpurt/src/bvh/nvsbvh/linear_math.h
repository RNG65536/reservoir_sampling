...
struct Vec2f
{
    ...
    inline __host__ __device__ float& operator[](int n) { return _v[n]; }
    inline __host__ __device__ const float& operator[](int n) const { return _v[n]; }
...
struct Vec2i
{
    ...
    inline __host__ __device__ int& operator[](int n) { return _v[n]; }
    inline __host__ __device__ const int& operator[](int n) const { return _v[n]; }
...
struct Vec4i
{
    ...
    inline __host__ __device__ int& operator[](int n) { return (&x)[n]; }
    inline __host__ __device__ const int& operator[](int n) const { return (&x)[n]; }
};
...
FI HaD float max1f(const float& a, const float& b)
{
    return (a < b) ? b : a;
}
FI HaD float min1f(const float& a, const float& b)
{
    return (a > b) ? b : a;
}
...
public:
    ...
    explicit FI HaD Vec2f(float a)
    {
        x = y = a;
    }
    FI HaD Vec2f(float _x, float _y)
    {
        x = _x;
        y = _y;
    }
    FI HaD Vec2f(const float2& v) : float2(v)
    {
    }
    FI HaD Vec2f& operator=(const float2& v)
    {
        x = v.x;
        y = v.y;
        return *this;
    ...
    FI HaD bool operator==(const Vec2f& v)
    {
        return x == v.x && y == v.y;
    }
    FI HaD Vec2f operator+(const Vec2f& v) const
    {
...
struct Vec2i
{
    ...
    FI HaD Vec2i(int _x = 0, int _y = 0) : x(_x), y(_y)
    {
    }
    FI HaD Vec2i(const Vec2i& v) : x(v.x), y(v.y)
    {
    }

    FI HaD bool operator==(const Vec2i& v)
    {
        return x == v.x && y == v.y;
    }
};
...
public:
    ...
    explicit FI HaD Vec3f(float a)
    {
        x = y = z = a;
    }
    FI HaD Vec3f(const Vec2f& v, float z_)
    {
        x = v.x;
        y = v.y;
        z = z_;
    }
    FI HaD Vec3f(float _x, float _y, float _z)
    {
        x = _x;
        y = _y;
        z = _z;
    }
    FI HaD Vec3f(const float3& v) : float3(v)
    {
    }
    FI HaD Vec3f& operator=(const float3& v)
    {
        x = v.x;
        y = v.y;
        z = v.z;
    ...
    FI HaD Vec3f normalized()
    {
        float norm = 1.0f / sqrtf(x * x + y * y + z * z);
        return Vec3f(x * norm, y * norm, z * norm);
    }
    FI HaD Vec3f& operator+=(const Vec3f& v)
    {
        x += v.x;
        y += v.y;
        z += v.z;
    ...
    FI HaD Vec3f& operator*=(const float& a)
    {
        x *= a;
        y *= a;
        z *= a;
    ...
    FI HaD Vec3f operator*(float a) const
    {
    ...
    FI HaD Vec3f operator*(const Vec3f& v) const
    {
    ...
    FI HaD bool operator!=(const Vec3f& v)
    {
...
struct Vec3i
{
    ...
    FI HaD Vec3i(int _x = 0, int _y = 0, int _z = 0)
        : x(_x), y(_y), z(_z)
    {
    }
    FI HaD Vec3i(const Vec3i& v) : x(v.x), y(v.y), z(v.z)
    {
    }
    FI HaD Vec3i(const Vec3f& vf)
        : x(vf.x), y(vf.y), z(vf.z)
    {
    }

    FI HaD bool operator==(const Vec3i& v)
    {
        return x == v.x && y == v.y && z == v.z;
    }
};
...
struct Vec4f
{
    ...
    FI HaD Vec4f(float _x = 0, float _y = 0, float _z = 0, float _w = 0)
        : x(_x), y(_y), z(_z), w(_w)
    {
    }
    FI HaD Vec4f(const Vec4f& v) : x(v.x), y(v.y), z(v.z), w(v.w)
    {
    }
    FI HaD Vec4f(const Vec3f& v, const float a)
        : x(v.x), y(v.y), z(v.z), w(a)
    {
    }

    FI HaD Vec4f& operator+=(const Vec4f& v)
    {
        x += v.x;
        y += v.y;
        z += v.z;
...
struct Vec4i
{
    ...
    FI HaD Vec4i(int _x = 0, int _y = 0, int _z = 0, int _w = 0)
        : x(_x), y(_y), z(_z), w(_w)
    {
    }
    FI HaD Vec4i(const Vec4i& v) : x(v.x), y(v.y), z(v.z), w(v.w)
    {
    }
    FI HaD Vec4i(const Vec3i& v, const int a)
        : x(v.x), y(v.y), z(v.z), w(a)
    {
    }
};

FI HaD Vec3f min3f(const Vec3f& v1, const Vec3f& v2)
{
    return Vec3f(v1.x < v2.x ? v1.x : v2.x,
                 v1.y < v2.y ? v1.y : v2.y,
                 v1.z < v2.z ? v1.z : v2.z);
}
FI HaD Vec3f max3f(const Vec3f& v1, const Vec3f& v2)
{
    return Vec3f(v1.x > v2.x ? v1.x : v2.x,
                 v1.y > v2.y ? v1.y : v2.y,
                 v1.z > v2.z ? v1.z : v2.z);
}
//FI HaD Vec3f cross(const Vec3f& v1, const Vec3f& v2)
//{
//    return Vec3f(v1.y * v2.z - v1.z * v2.y,
//                 v1.z * v2.x - v1.x * v2.z,
//                 v1.x * v2.y - v1.y * v2.x);
//}
//FI HaD float dot(const Vec3f& v1, const Vec3f& v2)
//{
//    return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
//}
//FI HaD float dot(const Vec4f& v1, const Vec4f& v2)
//{
//    return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
//}
//FI HaD float distancesq(const Vec3f& v1, const Vec3f& v2)
//{
//    return (v1.x - v2.x) * (v1.x - v2.x) + (v1.y - v2.y) * (v1.y - v2.y) +
//           (v1.z - v2.z) * (v1.z - v2.z);
//}
//FI HaD float distance(const Vec3f& v1, const Vec3f& v2)
//{
//    return sqrtf((v1.x - v2.x) * (v1.x - v2.x) + (v1.y - v2.y) * (v1.y - v2.y) +
//                 (v1.z - v2.z) * (v1.z - v2.z));
//}
//FI HaD Vec3f powf(const Vec3f& v1, const Vec3f& v2)
//{
//    return Vec3f(powf(v1.x, v2.x), powf(v1.y, v2.y), powf(v1.z, v2.z));
//}
//FI HaD Vec3f expf(const Vec3f& v)
//{
//    return Vec3f(expf(v.x), expf(v.y), expf(v.z));
//}
//FI HaD float clampf(float a, float lo, float hi)
//{
//    return a < lo ? lo : a > hi ? hi : a;
//}
//FI HaD Vec3f mixf(const Vec3f& v1, const Vec3f& v2, float a)
//{
//    return v1 * (1.0 - a) + v2 * a;
//}
//FI HaD float smoothstep(float edge0, float edge1, float x)
//{
//    float t;
