...
void Image::dump_ppm(const char* filename)
{
    ...
    for (int j = m_height - 1; j >= 0; --j)
    {
        for (int i = 0; i < m_width; ++i)
        {
            ...
            ofs << (unsigned char)(std::min(1.0f, m_buffer[offset + 0]) * 255)
                << (unsigned char)(std::min(1.0f, m_buffer[offset + 1]) * 255)
                << (unsigned char)(std::min(1.0f, m_buffer[offset + 2]) * 255);
        }
    }
    ofs.close();
}
...
HDRPixel toRGBE(const glm::vec3& c)
{
...
void Image::tonemap_reinhard()
{
    ...
    {
        lum[n] = luminance(glm::vec3(fb[n * 3], fb[n * 3 + 1], fb[n * 3 + 2]));
...
void Image::accumulate_pixel(int i, int j, const glm::vec3& c)
{
...
void Image::flip_updown()
{
    ...
    {
        for (int c = 0; c < m_width * 3; c++)
        {
            std::swap(m_buffer[r * m_width * 3 + c],
                      m_buffer[(m_height - 1 - r) * m_width * 3 + c]);
        }
    }
}
...
glm::vec3 Image::pixel(int i, int j) const
{
    return glm::vec3(m_buffer[(i + j * m_width) * 3 + 0],
